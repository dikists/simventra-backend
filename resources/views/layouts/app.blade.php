...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        @canany(['view maintenance', 'view incidents'])
        <div class="sidebar-section">Operasional & Audit</div>
        ...
        <a href="{{ route('maintenance.index') }}"
           class="sidebar-link {{ request()->routeIs('maintenance.*') ? 'active' : '' }}">
            ...
            @php
                $dueMaintCount = \App\Models\MaintenanceSchedule::where('is_active', true)->whereDate('next_due_date', '<=', now()->addDays(7))->count();
            @endphp
            @if($dueMaintCount > 0)
            <span class="sidebar-badge" style="background:#f46a6a;color:#fff;font-weight:700;">{{ $dueMaintCount }}</span>
            @endif
        </a>
        @endcan
        @can('view incidents')
        <a href="{{ route('incidents.index') }}"
           class="sidebar-link {{ request()->routeIs('incidents.*') ? 'active' : '' }}">
            ...
            @php
                $openIncCount = \App\Models\Incident::whereIn('status', ['open', 'investigating'])->count();
            @endphp
            @if($openIncCount > 0)
            <span class="sidebar-badge" style="background:#f1b44c;color:#fff;font-weight:700;">{{ $openIncCount }}</span>
            @endif
        </a>
        @endcan
        @endcanany
